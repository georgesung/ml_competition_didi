'''
Sweep hyperparameters to find the optimal settings for fully-connected neural network
'''
...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_float('learning_rate', 0.0006, 'Initial learning rate.')
...
def run_training(l2_scale_in, keep_prob_in, learning_rate_in):
	...
	# Create the final batch by concatenating the remaining test data, and some dummy data for padding
	x_test_final_batch = np.concatenate((x_test[last_batch_idx:], x_test[:num_pad]), axis=0)
	y_test_final_batch = np.concatenate((y_test[last_batch_idx:], y_test[:num_pad]), axis=0)
	# Finally, append this result to my list of test batches
	x_test_batches.append(x_test_final_batch)
	y_test_batches.append(y_test_final_batch)
	...
	with tf.Graph().as_default():
		...
		return (loss_value, test_mape)


def run_sweep():
	# sweep l2_scale, keep_prob
	l2_scales = [0.2, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001, 0.00005, 0.00001]
	...
	for i in range(num_iter):
		l2_scale = random.choice(l2_scales)
		keep_prob = random.choice(keep_probs)
		learning_rate = random.choice(learning_rates)

		print('iteration %d: l2_scale = %f, keep_prob = %f, learning_rate = %f' % (i, l2_scale, keep_prob, learning_rate))

		train_loss, test_mape = run_training(l2_scale, keep_prob, learning_rate)

		result_dict[(l2_scale, keep_prob, learning_rate)] = (train_loss, test_mape)

	print('Random sweep complete. result_dict:\n%s' % result_dict)
